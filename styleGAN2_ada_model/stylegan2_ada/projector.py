...
def project(
    ...
    *,
    ...
    logprint(f'Project image to {latent_space} latent space...')

    w_std = np.load('./w_std.npy')
    if latent_space=='wp':
        w_avg = np.load('./w_avg.npy').repeat(18, 1)
    elif latent_space=='w':
        w_avg = np.load('./w_avg.npy')
    else:
        print('wrong latent space type')
        return
...
@click.command()
@click.option('--network', 'network_pkl', help='Network pickle filename', default='../pretrain/StyleGAN2-ada-Generator.pth')
...
@click.option('--latent-space',           help='Use which latent space, w or wp', required=True, metavar='DIR')
def run_projection(
    network_pkl: str,
    target_fname: str,
    outdir: str,
    ...
    name = os.path.basename(target_fname).split('.')[0]
    ...
    synth_image = (synth_image + 1) * (255/2)
    synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
    PIL.Image.fromarray(synth_image, 'RGB').save(f'{outdir}/{name}_{latent_space}.png')
    print('latent code shape:',projected_w.shape)
    projected_w=projected_w.unsqueeze(0).cpu().numpy()
    print(np.sum(projected_w[:, 0, :] - projected_w[:, 1, :]))
    np.save(f'{outdir}/{name}_{latent_space}.npy',projected_w)
