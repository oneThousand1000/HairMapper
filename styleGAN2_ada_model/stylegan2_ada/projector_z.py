...
import glob
w_std = np.load('E:/data/ffhq-dataset/ffhq_gen_data/stylegan_ffhq_2_sample_each_layer/w_std.npy')

w_avg = np.load('E:/data/ffhq-dataset/ffhq_gen_data/stylegan_ffhq_2_sample_each_layer/w_avg.npy')#.repeat(18,1)
#print(w_avg.shape)
...
def project(
    ...
    *,
    ...
    z_init=rnd.randn(1,512)
    z_opt = torch.tensor(z_init, dtype=torch.float32, device=device, requires_grad=True)
    #
    ...
    for step in range(num_steps):
        ...
        # L2 loss
        # loss_pix=0.0
        # loss_pix = torch.mean(((target_images_origin - synth_images) ) ** 2, dim=[1, 2, 3])[0]*1e-4
        # loss += loss_pix
        ...
        #avg_loss =((w_avg_tensor - ws)).square().sum()/18
        #print(w_avg_tensor.size(),ws.size())
        # avg_loss =  (w_avg_tensor[:,:8,:]-ws[:,:8,:]).square().sum()
        # #print(np.sum(code[:,0,]-w_avg)**2,np.sum(code[:, 1:, ] - code[:,0,]) ** 2)
        # loss += avg_loss *5e-5 #1.5e-5
        ...
        loss.backward()
        optimizer.step()
        logprint(f'step {step+1:>4d}/{num_steps}: dist {dist:.4f}  reg_loss {reg_loss * regularize_noise_weight:.4f} loss {float(loss):.4f}')
...
def run_projection(
    ...
    f_list = glob.glob(f'F:/DoubleChin/datasets/ffhq_data/remove_hair/author/origin/*.png')
    import random
    ...
    for f_path  in f_list:
        ...
        if  (os.path.exists(f'{outdir}/code/{name}.npy')) or not(os.path.exists(f'F:/DoubleChin/datasets/ffhq_data/remove_hair/real_image/origin_code(encoded by encoder)/{name}.npy')):
            continue
        ...
        projected_w = project(
            ...
            verbose=True,
            vgg16=vgg16,
            #wp_init=np.load(f'F:/DoubleChin/datasets/ffhq_data/remove_hair/real_image/origin_code(encoded by encoder)/{name}.npy')[np.newaxis]
        )
