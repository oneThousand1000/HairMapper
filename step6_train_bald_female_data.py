...
def main():
    ...
    print(f'============= Training based on dataset {data_dir}, loading male mapper ckpt from {mapper_ckpt_path} =============')
    ...
    for sample_num,img_index in enumerate(female_index[-total_num:]):
        ...
        distance = np.abs(
            (np.sum(gender_boundarys * wp_latent_codes_origin, axis=2, keepdims=True) + gender_intercepts) / np.linalg.norm(
